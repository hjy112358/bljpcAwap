...
{    
    ...
    public function  __construct() {   
        ...
        $this->alipay_config['cacert']        = getcwd().'/plugins/payment/alipay/app_notify/cacert.pem'; //ca证书路径地址，用于curl中ssl校验 //请保证cacert.pem文件在当前文件夹目录中
    	...
    	if($verify_result){
    		...
    		if(intval($success_num)>0)
    		{
    			...
    			if($res[2] == 'SUCCESS'){
    				...
    				//修改订单商品状态
    				M('order_goods')->where(array('rec_id'=>$rec_id))->save(array('status'=>3));
    				$map = array('order_id'=>$order_goods['order_id'],'goods_id'=>$order_goods['goods_id']);
    				//更新退款申请状态
    				$updata = array('refund'=>$res[1],'refund_type'=>2,'refundtime'=>time());
    				M('return_goods')->where($map)->save($updata);
    			}
    		}
    		echo "success"; //告诉支付宝处理成功
    	}else{
