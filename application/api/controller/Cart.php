...
//        $user_id = I("user_id/d",0); // 用户id
        ...
        $storeList = M('store')->where("store_id in(select store_id from ".C('database.prefix')."cart where (user_id ={$where['user_id']} or session_id = '{$where['session_id']}')  )")->getField("store_id,store_name,is_own_shop"); // 找出商家
    ...
    public function cart2()
    {
        ...
        $sql = "select c1.name,c1.money,c1.condition, c2.* from __PREFIX__coupon as c1 inner join __PREFIX__coupon_list as c2  on c2.cid = c1.id and c1.type in(0,1,2,3) and order_id = 0
            where c2.uid = :uid  and ".time()." < c1.use_end_time and c1.condition <= {$cart_result['total_price']['total_fee']} and c2.store_id in (".implode(',', $store_id_arr).")";
        $couponList = Db::query($sql , ['uid'=>$this->user_id]);
    ...
    public function cart3(){
        ...
        $coupon_id        = $cart_form_data['coupon_id']; // $coupon_id =  I("coupon_id/d",0); //  优惠券id  数组形式
    ...
    {        
        ...
        $return_arr = array('status'=>1,'msg'=>'搜查成功','result'=>$sum_order_amount); // 返回结果状态
        exit(json_encode($return_arr));   
    }    
