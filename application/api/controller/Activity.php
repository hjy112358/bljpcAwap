...
class Activity extends Base {
    ...
    public function group_dyr(){
        ...
        if ($keys) {
            $specImage = M('SpecImage')->where(["goods_id" => $goods_id , 'src'=>['<>' ,'']])->getField("spec_image_id,src");// 规格对应的 图片表， 例如颜色
            $keys = str_replace('_', ',', $keys);
            $sql = "SELECT a.name,a.order,b.* FROM __PREFIX__spec AS a INNER JOIN __PREFIX__spec_item AS b ON a.id = b.spec_id WHERE b.id IN(:ids) ORDER BY a.order";
            $filter_spec2 = Db::query($sql , ['ids'=>$keys]);
            foreach ($filter_spec2 as $key => $val) {
