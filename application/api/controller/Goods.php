...
class Goods extends Base {
    ...
    public function goodsCategoryList(){        
        ...
        $goodsCategoryList = M('GoodsCategory')->where("parent_id = :parent_id AND is_show=1")->bind(['parent_id'=>$parent_id])->order("parent_id_path,sort_order desc")->select();
    ...
    public function goodsSecAndThirdCategoryList(){
     ...
     // SELECT * FROM `tp_goods_category` WHERE parent_id_path LIKE '%0\_1\_%'
      /** 一次查询所有二级和三级分类  **/
        ...
        $list = M('GoodsCategory')->where("parent_id_path LIKE :parent_id AND is_show=1")->bind(['parent_id'=>"%0_".$parent_id."_%"])->getField('id,mobile_name,image, level , parent_id');
        $list2 = array();
        foreach ($list as $k =>$v )
        {        
                if($v['level'] == 3)
    ...
    public function goodsList(){
    	...
    	$filter_goods_id = M('goods')->where(["is_on_sale"=>1,"cat_id".$goodsCate['level'] => $id])->cache(true)->getField("goods_id",true);
...
//         $list['filter_spec'] = array();
    ...
    public function goodsInfo(){
        ...
        $goods_attr_list = M('GoodsAttr')->where($where)->select(); // 查询商品属性表                        
        ...
        $keys = M('SpecGoodsPrice')->where($where)->getField("GROUP_CONCAT(`key` SEPARATOR '_') ");         
        if($keys)
        {
             ...
             $sql  = "SELECT a.name,a.order,b.* FROM __PREFIX__spec AS a INNER JOIN __PREFIX__spec_item AS b ON a.id = b.spec_id WHERE b.id IN(:key) ORDER BY a.order";
             $filter_spec2 = Db::query($sql,['key'=>$keys]);   
    ...
    */
    ...
    {        
        ...
        $_GET['p'] = $_REQUEST['p'];
        $page = new Page($count,10);        
        $list = M('comment')->where($where)->bind(['goods_id'=>$goods_id])->order("comment_id desc")->limit("{$page->firstRow},{$page->listRows}")->select();        
    ...
    function collectGoods(){
        ...
        //删除收藏商品
        if($type==1){
            M('GoodsCollect')->where("user_id",$this->user_id)->where('goods_id',$goods_id)->delete();
            exit(json_encode(array('status'=>1,'msg'=>'成功取消收藏','result'=>array() )));
        }
        $count = M('GoodsCollect')->where("user_id", $this->user_id)->where('goods_id',$goods_id)->count();
