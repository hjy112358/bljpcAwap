...
class Wxpay extends Base
{
    ...
    public function dopay()
    {
        ...
        $order_sn = I('order_sn','');
        if($master_order_sn){
            $amount = M('order')->where('master_order_sn' , $master_order_sn)->sum('order_amount');
        }else{
        ...
        $unifiedOrder->SetOut_trade_no($master_order_sn.$order_sn.time());//交易号 $master_order_sn.$order_sn 不能来个提示存在
