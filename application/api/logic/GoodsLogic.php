...
class GoodsLogic extends Model
{
    ...
    {            
        ...
        {
            ($cat['parent_id'] == 0) && $GoodsCategory->execute("UPDATE __PREFIX__goods_category set  parent_id_path = '0_$id', level = 1 where id = :id",['id'=>$id]); // 如果是一级分类          
            $GoodsCategory->execute("UPDATE __PREFIX__goods_category AS a ,tp_goods_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = :id",['id'=>$id]);         
