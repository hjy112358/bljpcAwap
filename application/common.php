...
function goods_thum_images($goods_id, $width, $height)
{
    ...
    $original_img = M('goods')->where('goods_id='.$goods_id)->find();
    return imgtransformation($original_img['original_img']);
...
function sendSms($scene, $sender, $params)
{
    ...
    $resp = realSendSMS($sender, $product, $smsParam, $code);
    if ($resp['status'] == 1) {
        ...
        $result = AlibabaCloud::rpcRequest()
            ...
            ->options([
                ...
                'query'                 => [
                    ...
                    'TemplateCode'      => $sms_templateCode,

                    'TemplateParam'     => json_encode(['code'=>$templateCode]),
...
function order_give($order)
{
    ...
    $points && accountLog($order['user_id'], 0, $points, "下单赠送积分", 0, $order['order_id']);
}
...
//处理一代关系
function firstrelate($user_id,$zcoupon){

    $res = Db::name('ticket_hold')->where('user_id',$user_id)->where('ticket_id',1)->find();
    if($res){

        if($res['ticket_number'] > ($zcoupon*0.48)){
            Db::name('ticket_hold')->where('user_id',$user_id)->where('ticket_id',1)->setDec('ticket_number',$zcoupon*0.48);

            $yue = Db::name('ticket_hold')->where('user_id',$user_id)->where('ticket_id',5)->find();
            if($yue){
                Db::name('ticket_hold')->where('user_id',$user_id)->where('ticket_id',5)->setInc('ticket_number',$zcoupon*0.48);
            }else{
                $ticket_last_change_time = date('Y-m-d H:i:s',time());
                Db::name('ticket_hold')->insertGetId(array('user_id'=>$user_id,'ticket_id'=>5,'ticket_number'=>$zcoupon*0.48,'ticket_last_change_time'=>$ticket_last_change_time));
            }
        }else{
            exit();
        }
...
function sendrelate($user_id,$zcoupon){
    $res = Db::name('ticket_hold')->where('user_id',$user_id)->where('ticket_id',1)->find();
    if($res){

        if($res['ticket_number'] > ($zcoupon*0.02)){
            Db::name('ticket_hold')->where('user_id',$user_id)->where('ticket_id',1)->setDec('ticket_number',$zcoupon*0.02);

            $yue = Db::name('ticket_hold')->where('user_id',$user_id)->where('ticket_id',5)->find();
            if($yue){
                Db::name('ticket_hold')->where('user_id',$user_id)->where('ticket_id',5)->setInc('ticket_number',$zcoupon*0.02);
            }else{
                $ticket_last_change_time = date('Y-m-d H:i:s',time());
                Db::name('ticket_hold')->insertGetId(array('user_id'=>$user_id,'ticket_id'=>5,'ticket_number'=>$zcoupon*0.02,'ticket_last_change_time'=>$ticket_last_change_time));
            }
        }
    }
...
function upload_oss($file){
    ...
    $accessKeyId = "LTAIdp9wXmHcZRy3";
    $accessKeySecret = "exSZIT6zNdSdG57btSPMzUlx6HYuYd";
...
function imgtransformation($url)
{
    if(strpos($url,'/public/') !== false){
        $arr = explode('/',$url);
        $img = $arr[count($arr)-1];
        $xurl = "https://2019-05-31.oss-cn-shanghai.aliyuncs.com/".$img;
        return $xurl;
    }else{
        return $url;
    }
