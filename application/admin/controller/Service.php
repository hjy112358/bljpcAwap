	...
	public function refund_info(){
		if(IS_POST){
			...
			if($data['refund_type'] == 1){
				...
				accountLog($return_goods['user_id'],$return_goods['refund'],0,'用户申请订单退款',0,$return_goods['order_id']);//处理退款
			}
			$data['type'] = 2;
	...
	public function complain_subject_info(){
		if(IS_POST){
			...
			if(M('complain_subject')->add($data)){
				$this->success('添加成功',U('Service/complain_subject_list'));exit;
			}else{
	...
	public function expose_type_info(){
		if(IS_POST){
			...
			if(M('expose_type')->add($data)){
				$this->success('添加成功',U('Service/expose_type_list'));exit;
			}else{
	...
	public function expose_subject_info(){
		if(IS_POST){
			...
			if(M('expose_subject')->add($data)){
				$this->success('添加成功',U('Service/expose_subject_list'));exit;
			}else{
