...
class Goods extends Base {
    ...
    public function addEditCategory(){
            ...
            if($_GET['is_ajax'] == 1)
            {
                ...
                $data['mobile_name'] = I('post.mobile_name');
                $parent_id_1 = I('post.parent_id_1');
                $parent_id_2 = I('post.parent_id_2');
                $data['is_show'] = I('post.is_show');
                $data['cat_group'] = I('post.cat_group');
                ...
                if($parent_id_2 == '0'){
                    if($parent_id_1 == '0'){
                        $data['parent_id'] = 0;
                        $data['level'] = 1;
                        $data['parent_id_path'] = '0_';
                    }else{
                        $data['parent_id'] = $parent_id_1;
                        $data['level'] = 2;
                        $data['parent_id_path'] = '0_'.$parent_id_1.'_';
                    }
                }else{
                        $data['parent_id'] =  $parent_id_2;
                        $data['level'] = 3;
                        $data['parent_id_path'] = '0_'.$parent_id_1.'_'.$parent_id_2.'_';
                }
                if($cztype == 'add'){
                   $cate_insertid = M('goods_category')->insertGetId($data);
                   if($cate_insertid){
                      $parent_id_path = $data['parent_id_path'].$cate_insertid;
                      $result = M('goods_category')->where('id',$cate_insertid)->save(
                        array('parent_id_path'=>$parent_id_path));
                      if($result){
                ...
                if($cztype == 'edit'){
                    $id = I('post.id');
                    $data['parent_id_path'] = $data['parent_id_path'].$id;
                    $cate_saveid = M('goods_category')->where('id',$id)->save($data);
                    if($cate_saveid){
