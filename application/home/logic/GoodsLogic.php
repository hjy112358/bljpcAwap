...
class GoodsLogic extends Model
{
   ...
   {       
        ...
        $list_brand = M('brand')->where("id in(select brand_id from ".C('database.prefix')."goods where brand_id > 0 and goods_id in($goods_id_str))")->limit('30')->select();  //where("goods_id in($goods_id_str)")->select();
...
//        $c3 = M('Comment')->where("goods_id = $goods_id and parent_id = 0 and  ceil(goods_rank) in(1,2)")->count(); // 差评
       $c1 = M('Comment')->where("is_show = 1 and  goods_id = $goods_id  and  ceil(goods_rank) in(4,5) and deleted = 0")->count(); // 好评
       $c2 = M('Comment')->where("is_show = 1 and goods_id = $goods_id  and  ceil(goods_rank) in(3) and deleted = 0")->count(); // 中评
       $c3 = M('Comment')->where("is_show = 1 and goods_id = $goods_id  and  ceil(goods_rank) in(1,2) and deleted = 0")->count(); // 差评
       $c4 = M('Comment')->where("is_show = 1 and  goods_id = $goods_id  and  img !='' and img NOT LIKE 'N;%'")->count(); // 晒图
    ...
    function getGoodsDispatching($goods_id,$region_id)
    {
        ...
        if(empty($goods['shipping_area_ids'])){
            ...
            foreach($plugin_goods_shipping as $k=>$v){
                $goods_shipping[$k]['freight'] = $cart_logic->cart_freight2($goods_shipping[$k]['shipping_code'],0,0,0,$goods['weight'] , $goods['store_id']);//默认全国
        ...
        $find_shipping_area_id = Db::name('area_region')->where(array('region_id'=>array('in',$parent_region_id)))->where('store_id', $goods['store_id'])->group('shipping_area_id')->getField('shipping_area_id',true);
        ...
        if(count($shipping_area_id_array) == 0){
            $goods_shipping = Db::name('shipping_area')->where(array('shipping_area_id'=>array('in',$goods_shipping_area_id_array),'is_default'=>1))->where('store_id', $goods['store_id'])->select();
