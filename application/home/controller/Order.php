...
class Order extends Base {
    ...
    public function order_detail(){
		...
		if($order_info['order_status'] == 3) {
			...
			$cancel_info['cancel_time'] = M('order_action')->where(array('order_id'=>$id))->find();
    ...
    public function complain_handle(){
    	...
    	if($complain_info){
    		...
    		$this->success('操作成功',U('Order/complain_list'));exit;
    	}else{
    ...
    public function expose(){
    	if(IS_POST){
    		...
    		$data['expose_user_name'] = $this->user['nickname'];
    ...
    public function get_expose_subject(){
    	...
    	if(empty($expose_subject)){
    		$subject = '<txt style="position: absolute; z-index: 2; line-height: 1; margin-left: 11px; margin-top: 11px; font-size: 13.3333px; font-family: monospace; color: rgb(205, 205, 205); display: inline;">请填写您认为该商品存在价格违规现象的理由</txt>
    				<textarea name="expose_content" id="note" cols="30" rows="10" style="border: 1px solid #E6E6E6;width: 935px; height: 144px;margin-bottom: 8px;padding: 5px;" placeholder=" "></textarea>
