...
class User extends Base
{
    ...
    */
    ...
    public function recharge()
    {
        ...
        $type = I('type');
        if ($type == 1) {
            $count2 = M('account_log')->where(array('user_id' => $this->user_id, 'user_money' => array('lt', 0)))->count();
            $Page2 = new Page($count2, 10);
            $consume_list = M('account_log')->where(array('user_id' => $this->user_id, 'user_money' => array('lt', 0)))->limit($Page2->firstRow . ',' . $Page2->listRows)->select();
            $this->assign('consume_list', $consume_list);//消费记录
            $this->assign('page2', $Page2->show());
            $this->assign('point_rate', tpCache('shopping.point_rate'));
        } else if ($type == 2) {
            $count = M('withdrawals')->where(array('user_id' => $this->user_id))->count();
            $Page = new Page($count, 10);
            $show = $Page->show();
            $recharge_list = M('withdrawals')->where(array('user_id' => $this->user_id))->limit($Page->firstRow . ',' . $Page->listRows)->select();
            $this->assign('page', $show);
            $this->assign('recharge_list', $recharge_list);//提现记录
        } else {
