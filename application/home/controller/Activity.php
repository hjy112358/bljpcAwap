...
//        $p = I('p',1);
    ...
    public function coupon_list(){
        $atype = I('atype/a',1);
        $count = M('coupon')->where(array('type'=>2,'status'=>1))->count();
        $Page = new Page($count,15);
        $show = $Page->show();
        $this->assign('page',$show);
        $coupon_list = M('coupon')->where(array('type'=>2,'status'=>1))->limit($Page->firstRow.','.$Page->listRows)->order('id desc')->select();
