...
function randnum(){
    ...
    //稀有号码
    $num_list = array('000001','111111','222222','333333','4444444','555555','6666666','777777','888888','999999');
    $nums = F('invitatecode');
    if(!$nums){
        $numscode = M('users')->field('invitatecode')->select();//查出已经使用的邀请码
        foreach ($numscode as $key => $value) {
            $numscode[$key] = $value['invitatecode'];
        }
        F('invitatecode',$num_list);
    ...
    $real_num = $zero.$num;
    while(!in_array($real_num,$num_list) && !in_array($real_num,$nums)){
        $num = rand('1','999999');
        $num_len = strlen($num);
        $zero = '';
        for($i=$num_len; $i<6; $i++){
