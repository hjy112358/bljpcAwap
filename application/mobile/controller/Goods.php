...
class Goods extends MobileBase {
    ...
    //     if($goodsCate)
    //         $filter_goods_id = M('goods')->where(" goods_state = 1 and is_on_sale=1 and cat_id3 = $id")->cache(true)->getField("goods_id",true);      
    //     else
    ...
    //         return $this->fetch();
        ...
        if($sort == 'shop_price'){
            $sort_asc = I('sort_asc');
            $order = "shop_price ".$sort_asc;

        }
   

        $goods_list = Db::name('goods')->where($where)->order($order)->select();
...
//        $show = $page->show();
    ...
    //   商品详情页
    public function goodsDetailNew(){	
        ...
        $userid = I('user_id');
        $yanzheng_partner = $this->yzpartner($userid,$goods_info);

        if($yanzheng_partner['status'] == '-1'){
            $this->error($yanzheng_partner['msg']);
        }

        $goods_images = Db::name('goods_images')->where('goods_id',$goods_id)->order('img_sort asc')->select();
        
        $goods_attr = Db::name('goods_attr')->where('goods_id',$goods_id)->where('attr_num','neq','0')->select();
        //查出券价
        $coupon_price = Db::name('coupon_price')->find();

        //查出商品的评价

        // $goods_comment = Db::name('comment')->where('')
        ...
        if($_SESSION['user']){
            ...
            $is_collect = Db::name('goods_collect')->where('goods_id',$goods_id)->where('user_id',$user_id)->find();
            if($is_collect){
                $is_collect = 1;
            }else{
               $is_collect = 0;
        ...
        //微信分享
        $wxapi  = new \app\api\controller\Wx();
        $config = $wxapi->config($goods_id);
    ...
    public function yzpartner($user_id,$data){
        if($data['is_partner'] == '1'){
            if($user_id == ''){
                return array('status'=>'-1','msg'=>'非法途径');
            }   
            //查用户信息

            $user_info = Db::name('users')->where('user_id',$user_id)->field('user_id,is_agree,partner_goodsid')->find();
            if($user_info['is_agree']!= 1){
                return array('status'=>-1,'msg'=>'请先同意合伙人协议');
            }

            if($user_info['partner_goodsid'] != ''){

                return array('status'=>-1,'msg'=>'您已经购买过合伙人商品了无需再次购买');
            }
    ...
    public function add_goodscollect(){
        if($_SESSION['user']){
            $user_id = $_SESSION['user']['user_id'];
            $goods_id = I('goods_id');
            ...
            if($is_collect){
                ...
                $result = Db::name('goods_collect')->where('goods_id',$goods_id)->where('user_id',$user_id)->delete();
                if($result){
                    return array('status'=>1,'msg'=>'取消收藏成功','type'=>2);
                }else{
                    return array('status'=>-1,'msg'=>'取消收藏失败');
                }
                ...
                $result = Db::name('goods_collect')->insertGetId($data);
                if($result){
                    return array('status'=>1,'msg'=>'收藏成功','type'=>1);
                }else{
                    return array('status'=>-1,'msg'=>'收藏失败');
                }
