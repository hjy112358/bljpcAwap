...
class GoodsLogic extends Model
{
    ...
    {
        $GoodsCategory = Db::name('goods_category'); // 实例化User对象
...
//            ($cat['parent_id'] == 0) && $GoodsCategory->execute("UPDATE __PREFIX__goods_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类
            if ($cat['parent_id'] == 0) {
                $GoodsCategory->where('id', $id)->update(['parent_id_path' => '0_' . $id]);
            }
    ...
    {
        $list = M('GoodsAttribute')->where(['type_id' => $type_id, 'attr_index' => ['gt', 0]])->order('`order` desc')->select();
        //$list = M('Spec')->where("1=1")->order('`order` desc')->select();        
        $str = '';
