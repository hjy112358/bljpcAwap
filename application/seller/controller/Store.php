...
class Store extends Base
{
    ...
    {   
        ...
        $list = Db::name('store_banner')->where('store_id',STORE_ID)->order('sort asc')->select();
    ...
    public function add_banner()
    {
        ...
        $id = I('id');
        if($id){
            $banner_detail = Db::name('store_banner')->where('id',$id)->find();
        ...
        if(IS_POST){
            if($_POST['id']){
                $result = Db::name('store_banner')->where('id',$id)->save($_POST);
            }else{
                
                $result = Db::name('store_banner')->insertGetId($_POST);
            }

            if($result){
                $this->success('操作成功','store/store_banner');
            }
    ...
    public function del_banner(){
        $id = I('id');
        $result = Db::name('store_banner')->where('id',$id)->delete();
        if($result){
            return 1;
        }else{
            return 2;
