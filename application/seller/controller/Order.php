...
class Order extends Base
{
    ...
    */
    ...
    public function order_print()
    {
        ...
        $store_info = Db::name('store')->where('store_id',$order['store_id'])->find();

        $shipping_info = Db::name('delivery_doc')->where('order_id',$order_id)->find();
    ...
    public function delivery_self()
    {
        ...
        $result = Db::name('order')->where('order_id',$order_id)->update([
            'shipping_status'=>'1',
            'is_self'=>'1'
        ]);
...
//        $sql = "select *,FROM_UNIXTIME(add_time,'%Y-%m-%d') as create_time from __PREFIX__order $where order by order_id";
    ...
    public function underorder(){
        ...
        if($pay_code <> ''){
            $where['a.pay_code'] = array('eq',$pay_code);
        }
        if($pay_status <> ''){
        ...
        if($add_time_begin <> ''){
            $where['a.add_time'][] = array('egt',$add_time_begin);
        }

        if($add_time_end <> ''){
            $where['a.add_time'][] = array('elt',$add_time_end);
        }

        $where['a.store_id'] = array('eq',STORE_ID);

        $where['a.under'] = array('eq',1);
