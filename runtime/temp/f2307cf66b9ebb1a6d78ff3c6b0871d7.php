<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:49:"./application/seller/new/goods\ajaxGoodsList.html";i:1491382652;}*/ ?>
<table class="ncsc-default-table">
    ...
    <tbody>
    <?php if(empty($goodsList) || ($goodsList instanceof \think\Collection && $goodsList->isEmpty())): ?>
        <tr>
            <td colspan="20" class="norecord"><div class="warning-option"><i class="icon-warning-sign"></i><span>暂无符合条件的数据记录</span></div></td>
        </tr>
    <?php else: if(is_array($goodsList) || $goodsList instanceof \think\Collection): $i = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
        <tr class="bd-line">
            ...
            <td><img width="20" height="20" src="__PUBLIC__/images/<?php if($list[is_on_sale] == 1): ?>yes.png<?php else: ?>cancel.png<?php endif; ?>" onclick="changeTableVal2('goods','goods_id','<?php echo $list['goods_id']; ?>','is_on_sale',this)"/></td>
            <td><input  style="<?php if($list['store_count'] < $store_warning_storage): ?>color: red<?php endif; ?>" class="txt-cen" type="text" onkeyup="this.value=this.value.replace(/[^\d.]/g,'')" onpaste="this.value=this.value.replace(/[^\d.]/g,'')"  onchange="ajaxUpdateField2(this);" name="store_count" size="4" data-table="goods" data-id="<?php echo $list['goods_id']; ?>" value="<?php echo $list['store_count']; ?>"/></td>
            <td><?php echo date('Y-m-d H:i',$list['on_time']); ?></td>
            <td>
