		...
		<?php if(is_array($list) || $list instanceof \think\Collection): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$v): ?> 
            <div class="endorse_num">
                <div class="hend_endor">
		             <img src="<?php echo (isset($userList[$v['buy_user_id']]['head_pic']) && ($userList[$v['buy_user_id']]['head_pic'] !== '')?$userList[$v['buy_user_id']]['head_pic']:"__STATIC__/images/hend_none.png"); ?>" width="60" height="60" />
                </div>
                <div class="sec_endor">
                    <p>昵称：<span><?php echo (isset($userList[$v['buy_user_id']]['nickname']) && ($userList[$v['buy_user_id']]['nickname'] !== '')?$userList[$v['buy_user_id']]['nickname']:"--"); ?></span></p>
                    <p>电话：<span><?php echo (isset($userList[$v['buy_user_id']]['mobile']) && ($userList[$v['buy_user_id']]['mobile'] !== '')?$userList[$v['buy_user_id']]['mobile']:"--"); ?></span></p>
                    <p>订单号：<span><?php echo $v['order_sn']; ?></span></p>
                    <p>会员ID：<span><?php echo $v['buy_user_id']; ?></span></p>
                </div> 
            </div>
		<?php endforeach; endif; else: echo "" ;endif; if(empty($list) || ($list instanceof \think\Collection && $list->isEmpty())): ?>
...
var  page = 1;
 ...
 {	 	 	 
		...
		$.ajax({
			type : "GET",			
			url:"/index.php?m=Mobile&c=Distribut&a=order_list&status=<?php echo $_GET['status']; ?>&is_ajax=1&p="+page,//+tab,
