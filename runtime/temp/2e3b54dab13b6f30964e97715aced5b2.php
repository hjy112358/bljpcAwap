<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:48:"./application/seller/new/order\ajaxdelivery.html";i:1561511155;}*/ ?>
<style>
...
<?php else: if(is_array($orderList) || $orderList instanceof \think\Collection): $i = 0; $__LIST__ = $orderList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
    ...
    <tr>
      <th colspan="21"><span class="ml5">订单编号：<strong><?php echo $list['order_sn']; ?></strong></span><span>下单时间：<em class="goods-time"><?php echo date('Y-m-d H:i',$list['add_time']); ?></em></span>
                ...
                <a href="<?php echo U('Order/order_print',array('order_id'=>$list['order_id'],'template'=>'picking')); ?>" target="_blank" class="ncbtn-mini" title="打印配货单"><i class="icon-print"></i>打印配货单</a></span>
      </th>
   	 </tr>
   	 <?php $goodsList = $goodsArr[$list['order_id']]; if(is_array($goodsList) || $goodsList instanceof \think\Collection): $k = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$good): $mod = ($k % 2 );++$k;if($k == 1): ?>
     	<!--  第一行 -->
     	<tr>
	      <td class="bdl w10"></td>
	      <td class="w50"><div class="pic-thumb"><a href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$good['goods_id'])); ?>" target="_blank"><img src="<?php echo goods_thum_images($good['goods_id'],240,240); ?>" /></a></div></td>
	      <td class="tl">
	      	<dl class="goods-name">
	      ...
	      <td class="bdl bdr order-info w500" rowspan="<?php echo count($goodsArr[$list['order_id']]); ?>">
	        ...
	        <dl>
	          <dt>运费：</dt>
	          <dd><?php if(($list['shipping_price'] < 0.01)): ?>（免运费）<?php else: ?>邮费:<?php echo $list['shipping_price']; endif; if($list['shipping_status'] != 1): ?>
					<span><a href="<?php echo U('Seller/Order/delivery_self',array('order_id'=>$list['order_id'])); ?>" class="ncbtn-mini ncbtn-mint fr"><i class="icon-truck"></i>自提发货&nbsp;&nbsp;</a></span>
