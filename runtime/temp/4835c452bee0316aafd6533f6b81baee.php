<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:45:"./application/seller/new/order\ajaxindex.html";i:1491382652;}*/ ?>
<table class="ncsc-default-table order">
  ...
  <tbody>
    ...
    <tr>
      <th colspan="20">
      		<span class="ml10">订单编号：<em><?php echo $list['order_sn']; ?></em></span> <span>下单时间：<em class="goods-time"><?php echo date('Y-m-d H:i',$list['add_time']); ?></em></span>
      		<span class="fr mr10"><a href="<?php echo U('Order/order_print',array('order_id'=>$list['order_id'],'template'=>'picking')); ?>" target="_blank" class="ncbtn-mini" title="打印配货单"><i class="icon-print"></i>打印配货单</a></span>
       </th>
    </tr>
    ...
    <?php $goodsList = $goodsArr[$list['order_id']]; if(is_array($goodsList) || $goodsList instanceof \think\Collection): $k = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$good): $mod = ($k % 2 );++$k;if($k == 1): ?>
     	...
     	<tr>
	      ...
	      <td class="bdl" rowspan="<?php echo count($goodsArr[$list['order_id']]); ?>">
	      	<p class="ncsc-order-amount"><?php echo $list['total_price']; ?></p>
	      ...
	      <td class="nscs-table-handle" rowspan="<?php echo count($goodsArr[$list['order_id']]); ?>">
	      	...
	      	<?php if(($list['order_status'] == 3) or ($list['order_status'] == 5)): ?>
